    ...
    <main>
      ...
      >
        ...
        <template v-slot:[`item.respond_message`]="{ item }">
          <template>
            <h6 v-if="!item.respond_message">-</h6>
            <div class="actions" v-else>
              <button
                class="btn_show"
                @click="showReplayModal(item.respond_message)"
              >
                <i class="fal fa-file-alt"></i>
              </button>
            </div>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <div class="actions">
            <a-tooltip
              placement="bottom"
              v-if="$can('contactus delete', 'contactus')"
            >
              <template slot="title">
  ...
  methods: {
    ...
    async confirmDeleteItem() {
      try {
        await this.$axios({
          method: "DELETE",
          url: `contact-us/${this.itemToDelete.id}`,
        });
